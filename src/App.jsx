...
// ─── FLAGS ────────────────────────────────────────────────────────────────────
const FLAG_URL = t => `https://cdn.jsdelivr.net/gh/hampusborgos/country-flags@main/svg/${({
...
function Flag({ team, size = 28 }) {
  ...
  const src = FLAG_URL(team);
  if (!src || err) return <span style={{ fontSize: size * 0.8, lineHeight: 1 }}>{EMOJI[team] || "⚽"}</span>;
  return <img src={src} alt={team} onError={() => setErr(true)} style={{ width: size * 1.45, height: size, objectFit: "cover", borderRadius: 3, border: "1px solid #e5e7eb", display: "block", flexShrink: 0 }} />;
}
...
function etToMYT(dateStr, timeStr) {
  ...
  const mh = Math.floor(tod / 60), mm = tod % 60, ampm = mh >= 12 ? "PM" : "AM";
...
// ─── KNOCKOUT BRACKET (OFFICIAL FIFA 2026) ────────────────────────────────────
// Best 3rd eligible groups per match (official FIFA matrix)
const BEST3RD_GROUPS = {
  74: ["A","B","C","D","F"],
  77: ["C","D","F","G","H"],
  79: ["C","E","F","H","I"],
  80: ["E","H","I","J","K"],
...
// ─── STANDINGS FROM PICKS ─────────────────────────────────────────────────────
function computeStandings(preds) {
  const groups = {};
  GROUP_MATCHES.forEach(m => {
    if (!groups[m.grp]) groups[m.grp] = {};
    [m.home, m.away].forEach(t => {
      if (!groups[m.grp][t]) groups[m.grp][t] = { team: t, p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 };
    });
    const r = preds[m.id];
    if (!r) return;
    const home = groups[m.grp][m.home], away = groups[m.grp][m.away];
    home.p++; away.p++;
    if (r === "home") { home.w++; home.pts += 3; away.l++; home.gf++; away.ga++; }
    else if (r === "away") { away.w++; away.pts += 3; home.l++; away.gf++; home.ga++; }
    else { home.d++; away.d++; home.pts++; away.pts++; home.gf++; away.gf++; home.ga++; away.ga++; }
  });
  const sorted = {};
  Object.keys(groups).forEach(g => {
    sorted[g] = Object.values(groups[g]).sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf || b.w - a.w);
  });
  return sorted;
}

// ─── RESOLVE KNOCKOUT TEAM FROM USER PICKS ────────────────────────────────────
function resolveTeam(slot, standings, userKO) {
  if (!slot) return null;
  if (slot.type === "winner" && standings[slot.grp]) return standings[slot.grp][0]?.team || null;
  if (slot.type === "runner" && standings[slot.grp]) return standings[slot.grp][1]?.team || null;
  if (slot.type === "best3rd") return userKO[`b3_${slot.groups.join("")}`] || null;
  if (slot.type === "winnerOf") return userKO[`w_${slot.matchId}`] || null;
  if (slot.type === "loserOf") return userKO[`l_${slot.matchId}`] || null;
  return null;
}

function buildKnockoutMatches(standings, userKO) {
  return KNOCKOUT_DEF.map(m => {
    const myt = etToMYT(m.date, m.et);
    const home = resolveTeam(m.homeSlot, standings, userKO);
    const away = resolveTeam(m.awaySlot, standings, userKO);
    return { ...m, ...myt, home, away, homeSlot: m.homeSlot, awaySlot: m.awaySlot };
  });
}
...
// ─── SCORING ──────────────────────────────────────────────────────────────────
function calcScore(userPreds, userKO, results, resultsKO) {
  let correct = 0, total = 0;
  GROUP_MATCHES.forEach(m => {
    if (userPreds[m.id] && results[m.id]) { total++; if (userPreds[m.id] === results[m.id]) correct++; }
  });
  KNOCKOUT_DEF.forEach(m => {
    const up = userKO[`w_${m.id}`], rp = resultsKO[`w_${m.id}`];
    if (up && rp) { total++; if (up === rp) correct++; }
    if (m.stage === "3P") {
      const up2 = userKO[`w_${m.id}`], rp2 = resultsKO[`w_${m.id}`];
      if (up2 && rp2) { total++; if (up2 === rp2) correct++; }
    }
  });
...
const STAGE_COLOR = { R32: "#7c3aed", R16: "#b45309", QF: "#b45309", SF: "#dc2626", "3P": "#4b5563", F: "#1d4ed8" };
...
const S = {
  ...
  wrapNarrow: { maxWidth: 460, margin: "0 auto", padding: "0 1rem 4rem" },
  hero: { background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "2rem 1.5rem 0", textAlign: "center" },
  heroTitle: { fontSize: 22, fontWeight: 700, color: "#111827", letterSpacing: -0.3 },
  ...
  statL: { fontSize: 10, color: "#9ca3af", marginTop: 2, textTransform: "uppercase", letterSpacing: "0.5px" },
  nav: { background: "#fff", borderBottom: "1px solid #e5e7eb", display: "flex", overflowX: "auto" },
  navTab: { flex: 1, minWidth: 70, padding: "11px 6px", fontSize: 11, fontWeight: 600, border: "none", background: "none", color: "#6b7280", cursor: "pointer", borderBottom: "2px solid transparent", whiteSpace: "nowrap", transition: "all 0.15s", textTransform: "uppercase", letterSpacing: "0.4px" },
  ...
  btnFull: { width: "100%" },
  alertY: { background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: "9px 12px", fontSize: 13, color: "#92400e", marginBottom: 10 },
  alertG: { background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 8, padding: "9px 12px", fontSize: 13, color: "#15803d", marginBottom: 10 },
  alertR: { background: "#fff5f5", border: "1px solid #fca5a5", borderRadius: 8, padding: "9px 12px", fontSize: 13, color: "#b91c1c", marginBottom: 10 },
  alertB: { background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: "9px 12px", fontSize: 13, color: "#1d4ed8", marginBottom: 10 },
  ...
  lbRow: { display: "flex", alignItems: "center", gap: 10, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: "11px 13px", marginBottom: 7 },
...
export default function App() {
  ...
  const [groupPreds, setGroupPreds] = useState({});  // {matchId: "home"|"away"|"draw"}
  const [userKO, setUserKO] = useState({});           // {w_matchId: team, b3_GROUPS: team}
  const [allUsers, setAllUsers] = useState({});
  const [results, setResults] = useState({});         // {matchId: "home"|"away"|"draw"}
  const [resultsKO, setResultsKO] = useState({});     // {w_matchId: team}
  ...
  useEffect(() => {
    (async () => {
      ...
      try { const r = await storageGet(RESULTS_KEY); if (r?.value) { const d = JSON.parse(r.value); setResults(d.group || {}); setResultsKO(d.ko || {}); } } catch {}
  ...
  const saveResults = async (grp, ko) => { await storageSet(RESULTS_KEY, JSON.stringify({ group: grp, ko })); setResults(grp); setResultsKO(ko); };
  const saveSettings = async s => { await storageSet(SETTINGS_KEY, JSON.stringify(s)); setSettings(s); };

  const standings = computeStandings(groupPreds);
  const koMatches = buildKnockoutMatches(standings, userKO);

  const totalPicks = (() => {
    let done = GROUP_MATCHES.filter(m => groupPreds[m.id]).length;
    koMatches.forEach(m => { if (userKO[`w_${m.id}`]) done++; });
    KNOCKOUT_DEF.filter(m => m.awaySlot?.type === "best3rd").forEach(m => {
      const key = `b3_${m.awaySlot.groups.join("")}`;
      if (userKO[key]) done++;
    });
    return { done, total: 104 + KNOCKOUT_DEF.filter(m => m.awaySlot?.type === "best3rd").length };
  })();

  const leaderboard = Object.entries(allUsers).map(([name, data]) => {
    const score = calcScore(data.groupPreds || {}, data.userKO || {}, results, resultsKO);
    return { name, pickCount: Object.keys(data.groupPreds || {}).length + Object.keys(data.userKO || {}).length, ...score };
  ...
  function handleLogin() {
    ...
    setUserKO(ex?.userKO || {});
    setTab("group"); setView("predict");
  }

  async function handleSave() {
    const upd = { ...allUsers, [username]: { groupPreds, userKO, savedAt: new Date().toISOString() } };
  ...
  function setGroupPick(id, val) { if (!isClosed) setGroupPreds(p => ({ ...p, [id]: val })); }
  function setKOPick(key, val) { if (!isClosed) setUserKO(p => ({ ...p, [key]: val })); }
  ...
  return (
    <div style={S.page}>
      ...
      {confirmDelete && (
        <div style={S.overlay}>
          <div style={S.confirmBox}>
            ...
            <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 20, lineHeight: 1.5 }}>This will permanently delete <strong>{confirmDelete}</strong>'s predictions.</div>
      ...
      {view === "home" && <HomeScreen nameInput={nameInput} setNameInput={setNameInput} onLogin={handleLogin} isClosed={isClosed} count={Object.keys(allUsers).length} resultsIn={Object.keys(results).length} onLB={() => setView("leaderboard")} deadline={deadline} regLocked={settings.registrationLocked} />}

      {view === "adminLogin" && (
        <div style={{ ...S.wrapNarrow, paddingTop: "3rem" }}>
          <div style={S.card}>
      ...
      {view === "admin" && <AdminPanel results={results} resultsKO={resultsKO} settings={settings} allUsers={allUsers} leaderboard={leaderboard} standings={computeStandings(results)} onSaveResults={saveResults} onSaveSettings={saveSettings} onDeleteUser={name => setConfirmDelete(name)} onBack={() => setView("home")} showToast={showToast} />}

      {view === "leaderboard" && <LeaderboardScreen leaderboard={leaderboard} resultsIn={Object.keys(results).length} onBack={() => setView(username ? "predict" : "home")} count={Object.keys(allUsers).length} />}
        ...
        <>
          <div style={S.header}>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#111827" }}>{username}</div>
                <div style={{ fontSize: 11, color: "#6b7280" }}>{totalPicks.done} picks · {Math.round(totalPicks.done / totalPicks.total * 100)}% done</div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <button style={{ ...S.btn, ...S.btnGhost, ...S.btnSm }} onClick={() => setView("leaderboard")}>Board</button>
          ...
          <div style={S.pbarWrap}><div style={{ ...S.pbar, width: `${Math.round(totalPicks.done / totalPicks.total * 100)}%` }} /></div>

          <div style={S.nav}>
            {[["group","Group Stage"],["standings","My Standings"],["bracket","Bracket"],["list","Match List"]].map(([t, l]) => (
              <button key={t} style={{ ...S.navTab, ...(tab === t ? S.navTabOn : {}) }} onClick={() => setTab(t)}>{l}</button>
            ))}
          </div>
          ...
          <div style={S.wrap}>
            ...
            {tab === "group" && <GroupStageTab groupPreds={groupPreds} onPick={setGroupPick} isClosed={isClosed} results={results} />}
            {tab === "standings" && <MyStandingsTab standings={standings} groupPreds={groupPreds} />}
            {tab === "bracket" && <BracketTab standings={standings} userKO={userKO} setKOPick={setKOPick} isClosed={isClosed} resultsKO={resultsKO} groupPreds={groupPreds} />}
            {tab === "list" && <ListTab standings={standings} userKO={userKO} setKOPick={setKOPick} isClosed={isClosed} resultsKO={resultsKO} groupPreds={groupPreds} />}
          </div>
        </>
      )}
    </div>
  ...
  return (
    <>
      <div style={S.hero}>
        ...
        <div style={S.heroTitle}>World Cup 2026 Prediction Challenge</div>
        <div style={S.heroSub}>FIFA World Cup 2026 · All times in Malaysia Time (MYT)</div>
        <div style={S.statsRow}>
          {[["104","Matches"],["48","Nations"],["3","Hosts"],[count || "0","Players"]].map(([v, l], i) => (
            <div key={l} style={{ ...S.statCell, borderRight: i < 3 ? "1px solid #f3f4f6" : "none" }}>
              <div style={S.statN}>{v}</div><div style={S.statL}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ ...S.wrapNarrow, paddingTop: "1.5rem" }}>
        {resultsIn > 0 && <div style={S.alertG}>📊 {resultsIn} results entered — leaderboard is live!</div>}
        {isClosed && <div style={S.alertR}>🔒 Predictions closed — tournament is underway!</div>}
        {regLocked && !isClosed && <div style={S.alertY}>🔒 Registration is closed. Contact the admin.</div>}
        <div style={S.card}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 10 }}>Enter your name to play</div>
          <input style={{ ...S.inp, marginBottom: 10 }} value={nameInput} onChange={e => setNameInput(e.target.value)} onKeyDown={e => e.key === "Enter" && onLogin()} placeholder='Your name — or "admin" for admin panel' autoFocus />
          <button style={{ ...S.btn, ...S.btnPrimary, ...S.btnFull }} onClick={onLogin}>{isClosed ? "View my predictions" : "Make my predictions →"}</button>
          <div style={{ marginTop: 8, fontSize: 11, color: "#9ca3af", textAlign: "center" }}>Predictions close: {mytD} MYT</div>
        </div>
        <button style={{ ...S.btn, ...S.btnGhost, ...S.btnFull, marginBottom: 14 }} onClick={onLB}>View Leaderboard</button>
        <div style={S.card}>
          <div style={S.cardTitle}>How to play</div>
          {[["⚽","Pick win / draw / loss for all 72 group stage matches"],["📊","View your predicted group standings — see who you're sending through"],["🏆","Fill in the full knockout bracket all the way to the Final"],["🔒","All picks lock at first kickoff — Jun 11, 3PM MYT"],["🥇","Highest accuracy % across all 104 matches wins"]].map(([ic, tx]) => (
...
// ─── GROUP STAGE TAB ──────────────────────────────────────────────────────────
function GroupStageTab({ groupPreds, onPick, isClosed, results }) {
  ...
  return (
    <div style={{ paddingTop: "1rem" }}>
      <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: "1rem" }}>
        {GROUPS.map(g => {
          const gDone = GROUP_MATCHES.filter(m => m.grp === g && groupPreds[m.id]).length;
          const gTotal = GROUP_MATCHES.filter(m => m.grp === g).length;
          const complete = gDone === gTotal;
          return (
            <button key={g} onClick={() => setActiveGrp(g)} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, border: `1px solid ${activeGrp === g ? "#2563eb" : complete ? "#86efac" : "#e5e7eb"}`, background: activeGrp === g ? "#2563eb" : complete ? "#f0fdf4" : "#fff", color: activeGrp === g ? "#fff" : complete ? "#15803d" : "#374151", cursor: "pointer" }}>
              {g} {complete ? "✓" : `${gDone}/${gTotal}`}
            </button>
          );
        })}
      </div>
      <div style={{ fontSize: 12, color: "#6b7280", marginBottom: "1rem" }}>{done}/72 group picks complete</div>
      {matches.map(m => <GroupMatchCard key={m.id} match={m} pick={groupPreds[m.id]} result={results[m.id]} onPick={v => onPick(m.id, v)} isClosed={isClosed} />)}
    </div>
  );
}

function GroupMatchCard({ match, pick, result, onPick, isClosed }) {
  const pickCorrect = pick && result && pick === result;
  const pickWrong = pick && result && pick !== result;
  const short = t => t && t.length > 12 ? t.split(" ")[0] : t;
  const resultText = r => r === "home" ? match.home : r === "away" ? match.away : "Draw";

  return (
    <div style={{ background: "#fff", border: `1px solid ${pickCorrect ? "#86efac" : pickWrong ? "#fca5a5" : result ? "#fde68a" : "#e5e7eb"}`, borderRadius: 10, marginBottom: 8, overflow: "hidden", background: pickCorrect ? "#f0fdf4" : pickWrong ? "#fff5f5" : "#fff" }}>
      ...
      <div style={{ padding: "12px 14px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 36px 1fr", alignItems: "center", gap: 6, marginBottom: 10 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: "#111827", textAlign: "center" }}>{match.home}</div>
          </div>
          <div style={{ textAlign: "center", fontSize: 12, fontWeight: 600, color: "#d1d5db" }}>vs</div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            <Flag team={match.away} size={28} />
            <div style={{ fontSize: 12, fontWeight: 600, color: "#111827", textAlign: "center" }}>{match.away}</div>
          </div>
        </div>
        {!isClosed && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
            {[["home", short(match.home), "#eff6ff", "#bfdbfe", "#1d4ed8"], ["draw", "Draw", "#fffbeb", "#fde68a", "#92400e"], ["away", short(match.away), "#f5f3ff", "#ddd6fe", "#6d28d9"]].map(([val, lbl, bg, border, color]) => (
              <button key={val} onClick={() => onPick(val)} style={{ padding: "7px 4px", borderRadius: 7, border: `1px solid ${pick === val ? border : "#e5e7eb"}`, background: pick === val ? bg : "#f9fafb", color: pick === val ? color : "#6b7280", fontSize: 11, fontWeight: 600, cursor: "pointer", textAlign: "center" }}>
                {pick === val ? "✓ " : ""}{lbl}
              </button>
            ))}
          </div>
        ...
        {isClosed && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, color: "#6b7280" }}>Your pick: <strong style={{ color: "#111827" }}>{pick ? resultText(pick) : "—"}</strong></span>
            {result && pick && <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: pickCorrect ? "#dcfce7" : "#fee2e2", color: pickCorrect ? "#15803d" : "#b91c1c" }}>{pickCorrect ? "✓ Correct" : "✗ Wrong"}</span>}
          </div>
        )}
        {result && <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 5 }}>Result: {resultText(result)}{result !== "draw" ? " win" : ""}</div>}
      </div>
    </div>
  );
}
...
// ─── MY STANDINGS TAB ─────────────────────────────────────────────────────────
function MyStandingsTab({ standings, groupPreds }) {
  const totalPicked = GROUP_MATCHES.filter(m => groupPreds[m.id]).length;
  return (
    <div style={{ paddingTop: "1rem" }}>
      {totalPicked < 72 && <div style={S.alertY}>Pick all 72 group matches to see complete standings. {72 - totalPicked} remaining.</div>}
      <div style={{ fontSize: 12, color: "#6b7280", marginBottom: "1rem" }}>Based on your group stage picks. Top 2 in each group advance automatically.</div>
      {GROUPS.map(g => {
          ...
          <div key={g} style={{ ...S.cardSm, marginBottom: 10 }}>
                  ...
                  <tr key={t.team} style={{ background: i < 2 ? "#f0fdf4" : "transparent" }}>
                    <td style={{ ...S.td, fontWeight: i < 2 ? 600 : 400 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 12, color: i < 2 ? "#15803d" : "#374151" }}>{t.team}</span>
                      </div>
                    </td>
                    <td style={{ ...S.td, textAlign: "center" }}>{t.p}</td>
                    <td style={{ ...S.td, textAlign: "center" }}>{t.w}</td>
                    <td style={{ ...S.td, textAlign: "center" }}>{t.d}</td>
                    <td style={{ ...S.td, textAlign: "center" }}>{t.l}</td>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              {table.slice(0, 3).map((t, i) => (
                <div key={t.team} style={{ fontSize: 10, color: i === 2 ? "#6b7280" : "#15803d", display: "flex", alignItems: "center", gap: 4 }}>
                  <Flag team={t.team} size={12} />
                  <span>{i === 0 ? "1st" : i === 1 ? "2nd" : "3rd"}: {t.team}</span>
                </div>
              ))}
            </div>
          </div>
...
// ─── BRACKET TAB ──────────────────────────────────────────────────────────────
function BracketTab({ standings, userKO, setKOPick, isClosed, resultsKO, groupPreds }) {
  const koMatches = buildKnockoutMatches(standings, userKO);
  const byStage = {};
  koMatches.forEach(m => { if (!byStage[m.stage]) byStage[m.stage] = []; byStage[m.stage].push(m); });

  const STAGE_ORDER = ["R32","R16","QF","SF","F","3P"];
  const STAGE_COL_W = 130;
  const MATCH_H = { R32: 56, R16: 112, QF: 224, SF: 448, F: 896, "3P": 80 };
  const totalH = 16 * 56;

  const getWinner = (m) => userKO[`w_${m.id}`];

  return (
    <div style={{ paddingTop: "1rem" }}>
      <div style={{ fontSize: 12, color: "#6b7280", marginBottom: "1rem" }}>
        Scroll right to see the full bracket. Click a team to pick them as the match winner.
      </div>
      <div style={{ overflowX: "auto", paddingBottom: "1rem" }}>
        <div style={{ display: "flex", gap: 0, minWidth: STAGE_ORDER.length * (STAGE_COL_W + 20) }}>
          {STAGE_ORDER.map((stage, si) => {
            const matches = byStage[stage] || [];
            const mh = MATCH_H[stage] || 56;
            const isLast = si === STAGE_ORDER.length - 1;
            return (
              <div key={stage} style={{ display: "flex", gap: 0 }}>
                <div style={{ width: STAGE_COL_W }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.8px", textAlign: "center", padding: "0 4px 8px" }}>{STAGE_LABEL[stage] || stage}</div>
                  <div style={{ height: stage === "3P" ? undefined : totalH + "px", display: "flex", flexDirection: "column" }}>
                    {matches.map((m, mi) => {
                      const winner = getWinner(m);
                      const canPick = m.home && m.away && !isClosed;
                      return (
                        <div key={m.id} style={{ display: "flex", alignItems: "center", height: stage === "3P" ? "auto" : mh + "px", flexShrink: 0 }}>
                          <div style={{ width: "100%", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden", margin: "0 2px" }}>
                            {[m.home, m.away].map((team, ti) => {
                              const isWinner = winner === team;
                              const isLoser = winner && winner !== team;
                              return (
                                <div key={ti} onClick={() => canPick && team && setKOPick(`w_${m.id}`, team)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 7px", borderBottom: ti === 0 ? "1px solid #f3f4f6" : "none", background: isWinner ? "#eff6ff" : "transparent", cursor: canPick && team ? "pointer" : "default", opacity: isLoser ? 0.45 : 1, transition: "background 0.12s" }}>
                                  {team ? <Flag team={team} size={16} /> : <div style={{ width: 23, height: 16, background: "#f3f4f6", borderRadius: 2 }} />}
                                  <span style={{ fontSize: 10, fontWeight: isWinner ? 600 : 400, color: isWinner ? "#1d4ed8" : team ? "#111827" : "#9ca3af", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
                                    {team ? (team.length > 11 ? team.split(" ")[0] : team) : "TBD"}
                                  </span>
                                  {isWinner && <span style={{ fontSize: 9, color: "#2563eb" }}>✓</span>}
                                </div>
                              );
                            })}
                          </div>
                ...
                {!isLast && stage !== "3P" && (
                  <div style={{ width: 20, display: "flex", flexDirection: "column", height: totalH + "px" }}>
                    {matches.map((m, mi) => (
                      <div key={m.id} style={{ height: mh + "px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-start" }}>
      ...
      <div style={{ marginTop: "1rem" }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Best 3rd-place picks</div>
        <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 12 }}>Pick which 3rd-place team advances in each bracket slot.</div>
        {KNOCKOUT_DEF.filter(m => m.awaySlot?.type === "best3rd").map(m => (
          <Best3rdPicker key={m.id} matchDef={m} standings={standings} userKO={userKO} setKOPick={setKOPick} isClosed={isClosed} groupPreds={groupPreds} />
        ))}
      </div>
    </div>
  );
...
// ─── BEST 3RD PICKER ──────────────────────────────────────────────────────────
function Best3rdPicker({ matchDef, standings, userKO, setKOPick, isClosed, groupPreds }) {
  const groups = matchDef.awaySlot.groups;
  const key = `b3_${groups.join("")}`;
  const selected = userKO[key];
  const myt = etToMYT(matchDef.date, matchDef.et);

  const homeTeam = resolveTeam(matchDef.homeSlot, standings, userKO);

  const eligibleTeams = groups.map(g => {
    const table = standings[g];
    if (!table || table.length < 3) return null;
    const team = table[2];
    if (!team) return null;
    const grpMatches = GROUP_MATCHES.filter(m => m.grp === g);
    const record = grpMatches.map(m => {
      const pick = groupPreds[m.id];
      if (!pick) return null;
      const isHome = m.home === team.team;
      if (pick === "draw") return "D";
      if ((pick === "home" && isHome) || (pick === "away" && !isHome)) return "W";
      return "L";
    }).filter(Boolean);
    return { ...team, grp: g, record };
  }).filter(Boolean);

  return (
    <div style={{ ...S.card, marginBottom: 10 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 2 }}>
        Match {matchDef.id}: {STAGE_LABEL[matchDef.stage]} · {myt.date} · {myt.time} MYT
      </div>
      <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 10 }}>
        {homeTeam || `Winner Group ${matchDef.homeSlot.grp}`} vs Best 3rd from Groups {groups.join("/")}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {eligibleTeams.length === 0 && <div style={{ fontSize: 12, color: "#9ca3af" }}>Pick group stage matches first to see eligible teams.</div>}
        {eligibleTeams.map(t => {
          const isSelected = selected === t.team;
          const wdl = t.record;
          const w = wdl.filter(r => r === "W").length;
          const d = wdl.filter(r => r === "D").length;
          const l = wdl.filter(r => r === "L").length;
          return (
            <button key={t.team} onClick={() => !isClosed && setKOPick(key, t.team)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 8, border: `1px solid ${isSelected ? "#bfdbfe" : "#e5e7eb"}`, background: isSelected ? "#eff6ff" : "#f9fafb", cursor: isClosed ? "default" : "pointer", textAlign: "left" }}>
              <Flag team={t.team} size={24} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: isSelected ? 600 : 500, color: isSelected ? "#1d4ed8" : "#111827" }}>{t.team}</div>
                <div style={{ fontSize: 11, color: "#6b7280", marginTop: 1 }}>
                  Group {t.grp} 3rd · {t.pts} pts · {wdl.length > 0 ? `W${w} D${d} L${l}` : "no picks yet"}
                </div>
              </div>
              {isSelected && <span style={{ fontSize: 11, color: "#2563eb", fontWeight: 600 }}>✓ Selected</span>}
            </button>
          );
        })}
      </div>
...
// ─── LIST TAB (round by round) ────────────────────────────────────────────────
function ListTab({ standings, userKO, setKOPick, isClosed, resultsKO, groupPreds }) {
    ...
    <div style={{ paddingTop: "1rem" }}>
      <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: "1rem" }}>
        {["R32","R16","QF","SF","3P","F"].map(s => {
          const matches = koMatches.filter(m => m.stage === s);
          const done = matches.filter(m => userKO[`w_${m.id}`]).length;
          const complete = done === matches.length && matches.length > 0;
          return (
            <button key={s} onClick={() => setActiveStage(s)} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: `1px solid ${activeStage === s ? STAGE_COLOR[s] : complete ? "#86efac" : "#e5e7eb"}`, background: activeStage === s ? STAGE_COLOR[s] : complete ? "#f0fdf4" : "#fff", color: activeStage === s ? "#fff" : complete ? "#15803d" : "#374151", cursor: "pointer" }}>
              {STAGE_LABEL[s]} {complete ? "✓" : `${done}/${matches.length}`}
            </button>
          );
        })}
      </div>
      ...
      {stageMatches.map(m => (
        <KOMatchCard key={m.id} match={m} userKO={userKO} setKOPick={setKOPick} isClosed={isClosed} resultsKO={resultsKO} standings={standings} groupPreds={groupPreds} />
      ))}
...
function KOMatchCard({ match, userKO, setKOPick, isClosed, resultsKO, standings, groupPreds }) {
  const winner = userKO[`w_${match.id}`];
  const actualWinner = resultsKO[`w_${match.id}`];
  const pickCorrect = winner && actualWinner && winner === actualWinner;
  const pickWrong = winner && actualWinner && winner !== actualWinner;
  const canPick = match.home && match.away && !isClosed;

  return (
    <div style={{ background: pickCorrect ? "#f0fdf4" : pickWrong ? "#fff5f5" : "#fff", border: `1px solid ${pickCorrect ? "#86efac" : pickWrong ? "#fca5a5" : "#e5e7eb"}`, borderRadius: 10, marginBottom: 8, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 12px", background: "#f9fafb", borderBottom: "1px solid #f3f4f6" }}>
        <span style={{ fontSize: 10, fontWeight: 600, color: STAGE_COLOR[match.stage] || "#6b7280", textTransform: "uppercase" }}>{STAGE_LABEL[match.stage]} · Match {match.id}</span>
        <span style={{ fontSize: 11, color: "#9ca3af" }}>{match.time} MYT · {match.venue}</span>
      </div>
      <div style={{ padding: "12px 14px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 36px 1fr", alignItems: "center", gap: 6, marginBottom: match.home && match.away ? 10 : 0 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            {match.home ? <><Flag team={match.home} size={26} /><div style={{ fontSize: 12, fontWeight: 600, color: "#111827", textAlign: "center" }}>{match.home}</div></> : <div style={{ fontSize: 12, color: "#9ca3af" }}>TBD</div>}
          </div>
          <div style={{ textAlign: "center", fontSize: 12, fontWeight: 600, color: "#d1d5db" }}>vs</div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            {match.away ? <><Flag team={match.away} size={26} /><div style={{ fontSize: 12, fontWeight: 600, color: "#111827", textAlign: "center" }}>{match.away}</div></> : <div style={{ fontSize: 12, color: "#9ca3af" }}>TBD</div>}
          </div>
        </div>

        {canPick && match.home && match.away && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {[match.home, match.away].map((team, ti) => (
              <button key={ti} onClick={() => setKOPick(`w_${match.id}`, team)} style={{ padding: "7px 8px", borderRadius: 7, border: `1px solid ${winner === team ? "#bfdbfe" : "#e5e7eb"}`, background: winner === team ? "#eff6ff" : "#f9fafb", color: winner === team ? "#1d4ed8" : "#6b7280", fontSize: 12, fontWeight: winner === team ? 600 : 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                <Flag team={team} size={14} />
                {winner === team ? "✓ " : ""}{team.length > 12 ? team.split(" ")[0] : team}
              </button>
            ))}
          </div>
        )}
        ...
        {!canPick && winner && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, color: "#6b7280" }}>Your pick: <strong style={{ color: "#111827" }}>{winner}</strong></span>
            {actualWinner && <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: pickCorrect ? "#dcfce7" : "#fee2e2", color: pickCorrect ? "#15803d" : "#b91c1c" }}>{pickCorrect ? "✓ Correct" : "✗ Wrong"}</span>}
          </div>
        )}

        {(!match.home || !match.away) && (
          <div style={{ fontSize: 12, color: "#9ca3af", textAlign: "center" }}>Teams advance from previous round</div>
        )}
      </div>
    </div>
  );
...
// ─── ADMIN PANEL ──────────────────────────────────────────────────────────────
function AdminPanel({ results, resultsKO, settings, allUsers, leaderboard, standings, onSaveResults, onSaveSettings, onDeleteUser, onBack, showToast }) {
  const [adminTab, setAdminTab] = useState("results");
  const [activeGrp, setActiveGrp] = useState("A");
  const [activeKOStage, setActiveKOStage] = useState("R32");
  const [localSettings, setLocalSettings] = useState(settings);
  const [localResultsKO, setLocalResultsKO] = useState(resultsKO);

  const adminStandings = computeStandings(results);
  const adminKOMatches = buildKnockoutMatches(adminStandings, localResultsKO);

  async function saveGroupResult(id, val) {
    const r = { ...results, [id]: val };
    await onSaveResults(r, localResultsKO);
    showToast("Result saved");
  }

  async function clearGroupResult(id) {
    const r = { ...results }; delete r[id];
    await onSaveResults(r, localResultsKO);
    showToast("Result cleared");
  }

  async function saveKOResult(matchId, team) {
    const ko = { ...localResultsKO, [`w_${matchId}`]: team };
    setLocalResultsKO(ko);
    await onSaveResults(results, ko);
    showToast("Result saved");
  }

  async function clearKOResult(matchId) {
    const ko = { ...localResultsKO }; delete ko[`w_${matchId}`];
    setLocalResultsKO(ko);
    await onSaveResults(results, ko);
    showToast("Result cleared");
  }
    ...
    <>
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "#6b7280" }}>{Object.keys(results).length}/72 group results · {Object.keys(localResultsKO).length} knockout results · {Object.keys(allUsers).length} players</div>
        </div>
      </div>

      <div style={S.nav}>
        {[["results","Group Results"],["knockout","Knockout Results"],["standings","Standings"],["players","Players"],["settings","Settings"]].map(([t, l]) => (
      ...
      <div style={S.wrap}>
        {adminTab === "results" && (
          <>
            <div style={{ paddingTop: "1rem" }}>
              <div style={S.alertY}>Click a result to save instantly. Green = result entered.</div>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: "1rem" }}>
                {GROUPS.map(g => {
                  const done = GROUP_MATCHES.filter(m => m.grp === g && results[m.id]).length;
                  const total = GROUP_MATCHES.filter(m => m.grp === g).length;
                  return (
                    <button key={g} onClick={() => setActiveGrp(g)} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, border: `1px solid ${activeGrp === g ? "#2563eb" : done === total ? "#86efac" : "#e5e7eb"}`, background: activeGrp === g ? "#2563eb" : done === total ? "#f0fdf4" : "#fff", color: activeGrp === g ? "#fff" : done === total ? "#15803d" : "#374151", cursor: "pointer" }}>
                      {g} {done}/{total}
                    </button>
                  );
                })}
              ...
              {groupMatches.map(m => (
                <div key={m.id} style={{ ...S.cardSm }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: "#6b7280" }}>{m.date} · {m.time} MYT</div>
                    {results[m.id] && <button style={{ ...S.btn, ...S.btnDanger, ...S.btnSm }} onClick={() => clearGroupResult(m.id)}>Clear</button>}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <Flag team={m.home} size={18} />
                    <span style={{ fontSize: 13, fontWeight: 500 }}>{m.home}</span>
                    <span style={{ color: "#9ca3af", fontSize: 12 }}>vs</span>
                    <Flag team={m.away} size={18} />
                    <span style={{ fontSize: 13, fontWeight: 500 }}>{m.away}</span>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
                    {[["home", m.home], ["draw", "Draw"], ["away", m.away]].map(([val, lbl]) => (
                      <button key={val} onClick={() => saveGroupResult(m.id, val)} style={{ padding: "6px 4px", borderRadius: 6, border: `1px solid ${results[m.id] === val ? "#86efac" : "#e5e7eb"}`, background: results[m.id] === val ? "#f0fdf4" : "#f9fafb", color: results[m.id] === val ? "#15803d" : "#6b7280", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                        {results[m.id] === val ? "✓ " : ""}{lbl.length > 12 ? lbl.split(" ")[0] : lbl}
                      </button>
                    ))}
                  </div>
                </div>
        ...
        {adminTab === "knockout" && (
          <div style={{ paddingTop: "1rem" }}>
            <div style={S.alertB}>Enter the winning team for each knockout match. Teams auto-populate from group results.</div>
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: "1rem" }}>
              {["R32","R16","QF","SF","3P","F"].map(s => (
                <button key={s} onClick={() => setActiveKOStage(s)} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: `1px solid ${activeKOStage === s ? STAGE_COLOR[s] : "#e5e7eb"}`, background: activeKOStage === s ? STAGE_COLOR[s] : "#fff", color: activeKOStage === s ? "#fff" : "#374151", cursor: "pointer" }}>
                  {STAGE_LABEL[s]}
                </button>
              ))}
            </div>
            {koStageMatches.map(m => {
              const actualWinner = localResultsKO[`w_${m.id}`];
              return (
                <div key={m.id} style={S.cardSm}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: "#6b7280" }}>Match {m.id} · {m.date} · {m.time} MYT</div>
                    {actualWinner && <button style={{ ...S.btn, ...S.btnDanger, ...S.btnSm }} onClick={() => clearKOResult(m.id)}>Clear</button>}
                  </div>
                  {m.home && m.away ? (
                    <>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
                        {[m.home, m.away].map(team => (
                          <button key={team} onClick={() => saveKOResult(m.id, team)} style={{ padding: "6px 8px", borderRadius: 6, border: `1px solid ${actualWinner === team ? "#86efac" : "#e5e7eb"}`, background: actualWinner === team ? "#f0fdf4" : "#f9fafb", color: actualWinner === team ? "#15803d" : "#6b7280", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, justifyContent: "center" }}>
                            <Flag team={team} size={14} />{actualWinner === team ? "✓ " : ""}{team.length > 12 ? team.split(" ")[0] : team}
                          </button>
                        ))}
                      </div>
                    </>
        ...
        {adminTab === "standings" && (
          <div style={{ paddingTop: "1rem" }}>
            <div style={S.alertB}>Live standings from results entered. Top 2 advance to Round of 32.</div>
            {GROUPS.map(g => {
              const table = adminStandings[g];
              if (!table || table.length === 0) return <div key={g} style={{ ...S.cardSm, color: "#9ca3af", fontSize: 12 }}>Group {g} — no results yet</div>;
              return (
                <div key={g} style={{ ...S.cardSm, marginBottom: 10 }}>
                  <div style={S.cardTitle}>Group {g}</div>
                  <table style={S.table}>
        ...
        {adminTab === "players" && (
          <div style={{ paddingTop: "1rem" }}>
            ...
            <div style={S.alertY}>Deleting an entry is permanent and cannot be undone.</div>
              ...
              <div key={u.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: "#f9fafb", borderRadius: 8, marginBottom: 6, border: "1px solid #f3f4f6" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#9ca3af" }}>{u.pickCount} picks{u.pct !== null ? ` · ${u.pct}% accuracy` : ""}</div>
                  </div>
                </div>
        ...
        {adminTab === "settings" && (
          <div style={{ paddingTop: "1rem" }}>
            <div style={S.card}>
              <div style={S.cardTitle}>Prediction deadline (MYT)</div>
              <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>Predictions lock at this time. Enter as UTC time (MYT = UTC+8).</div>
              <input type="datetime-local" style={{ ...S.inp, marginBottom: 10 }} value={localSettings.deadline ? localSettings.deadline.slice(0, 16) : ""} onChange={e => setLocalSettings(s => ({ ...s, deadline: e.target.value + ":00Z" }))} />
              <div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 10 }}>
                Current: {localSettings.deadline ? new Date(localSettings.deadline).toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur", dateStyle: "medium", timeStyle: "short" }) + " MYT" : "Not set"}
              </div>
            </div>
            <div style={S.card}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: "#111827" }}>Lock new registrations</div>
                  <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>{localSettings.registrationLocked ? "New players cannot join" : "Anyone can register"}</div>
                </div>
                <label style={{ position: "relative", width: 44, height: 24, flexShrink: 0 }}>
                  <input type="checkbox" checked={localSettings.registrationLocked || false} onChange={e => setLocalSettings(s => ({ ...s, registrationLocked: e.target.checked }))} style={{ opacity: 0, width: 0, height: 0 }} />
                  <span style={{ position: "absolute", inset: 0, background: localSettings.registrationLocked ? "#2563eb" : "#d1d5db", borderRadius: 12, cursor: "pointer", transition: "0.3s" }}>
                    <span style={{ position: "absolute", height: 18, width: 18, left: localSettings.registrationLocked ? 23 : 3, bottom: 3, background: "#fff", borderRadius: "50%", transition: "0.3s" }} />
                  </span>
                </label>
              </div>
            </div>
            <div style={S.card}>
              ...
              <div style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.6 }}>Update the <code style={{ background: "#f3f4f6", padding: "1px 5px", borderRadius: 3, fontSize: 12 }}>ADMIN_PW</code> constant in App.jsx and redeploy to change.</div>
            </div>
            <button style={{ ...S.btn, ...S.btnPrimary, ...S.btnFull }} onClick={handleSaveSettings}>Save settings</button>
          </div>
        )}
      </div>
    </>
      ...
      <div style={S.wrap}>
        {resultsIn === 0 ? (
          <div style={{ ...S.card, textAlign: "center", padding: "2.5rem", marginTop: "1.25rem" }}>
            ...
            <div style={{ fontSize: 13, color: "#6b7280" }}>Accuracy % appears once admin enters match results</div>
          </div>
            ...
            {leaderboard.map((u, i) => (
              <div key={u.name} style={{ ...S.lbRow, borderColor: i === 0 && u.pct !== null ? "#93c5fd" : "#e5e7eb", background: i === 0 && u.pct !== null ? "#eff6ff" : "#fff" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>{u.pickCount} picks made</div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  {u.pct !== null
                    ...
                    : <><div style={{ fontSize: 13, color: "#6b7280", fontWeight: 600 }}>{u.pickCount} picks</div><div style={{ fontSize: 11, color: "#9ca3af" }}>awaiting results</div></>}
                </div>
              </div>
            ))}
          </div>
